    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="row g-3" method="POST"
                action="{{ $item && $item->id ? route('transportdatatypes.update', $item->id) : route('transportdatatypes.store') }}">
                @csrf
                <div class="col-md-6">
                    <label class="form-label" for="name">Название</label>
                    <input class="form-control" id="name" name="name"
                        value="{{ $item && $item->name ? $item->name : old('name') }}" type="text"
                        placeholder="Окта Страхование" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label" for="code">Код</label>
                    <input class="form-control" id="code" name="code"
                        value="{{ $item && $item->code ? $item->code : old('code') }}" type="text" placeholder="OCTA"
                        required>
                </div>
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="type" name="type">
                        @foreach (\App\Models\DataType::TYPE as $key => $value)
                            <option value="{{ $value }}" @if (($item && $item->type) || old('status'))
                                selected
                        @endif
                        >{{ $key }}</option>
                        @endforeach
